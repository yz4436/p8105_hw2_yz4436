...
value. Finally, remove prez_gop, prez_dem, and day. show_col_types =
FALSE to avoid warning.
...
Clean the data in snp.csv and do some steps as the same way for
pols-month.csv. Since the year in this dataset should be recalculated by
myself, I used ifelse function to change year like 10 to 2020 and 50 to
1950. The remove day variable to keep same as pols-month.csv. Then I
locate year in the first column to arrange and organize so that year and
month are the leading columns. show_col_types = FALSE to avoid warning.
...
Clean and tidy unemployment.csv to let it can be merged with previous
documents. I used pivot_longer to switching it from “wide” to “long”
format and ensured that key variables have the same name and key
variables take the same values. show_col_types = FALSE to avoid warning.
...
Use left_join to join the datasets by merging snp into pols as
snp_pols_merge, and merging unemployment into the result as
...
-   For pols-month.csv dataset, it contained 9 variables, which are
    year, month, gov_gop, sen_gop, rep_gop, gov_dem, sen_dem, rep_dem,
    president.
-   For snp.csv dataset, it contained 3 variables, which are year,
    month, close.
-   For unemployment.csv dataset, it contained 3 variables, which are
    year, month, percentage_of_unemployment.
-   For the resulting dataset snp_pols_unemployment, the dimension of it
    is 822 \* 11. The range of years is 1947, 2015. There are 11
    variables, which are year, month, gov_gop, sen_gop, rep_gop,
